...
namespace til // Terminal Implementation Library. Also: "Today I Learned"
{
    namespace io
    {
        namespace details
        {
            ...
            _TIL_INLINEPREFIX std::string read_file_as_utf8_string(const std::filesystem::path& path, const bool elevatedOnly, const bool throwOn404, FILETIME* lastWriteTime)
            {
                ...
                for (auto i = 0; i < 3; ++i)
                {
                    ...
                    if (!file)
                    {
                        ...
                        if (!throwOn404 && gle == ERROR_FILE_NOT_FOUND)
                        {
                            return {};
                        }
                        THROW_WIN32(gle);
        ...
        inline std::string read_file_as_utf8_string(const std::filesystem::path& path, const bool elevatedOnly = false, FILETIME* lastWriteTime = nullptr)
        {
            return details::read_file_as_utf8_string(path, evelatedOnly, true, lastWriteTime);
        }

        // Same as read_file_as_utf8_string, but returns an empty optional, if the file couldn't be opened.
        inline std::string read_file_as_utf8_string_if_exists(const std::filesystem::path& path, const bool elevatedOnly = false, FILETIME* lastWriteTime = nullptr)
        {
            return details::read_file_as_utf8_string(path, evelatedOnly, false, lastWriteTime);
        }
